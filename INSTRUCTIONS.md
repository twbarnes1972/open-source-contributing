...
### 2026-07-12 — Third intake (1 item — folded into ISSUE-0002)
Processed permission-probe's freshness report into the existing [ISSUE-0002](./tasks/open/ISSUE-0002.md) (no new task — it satisfies exactly what that task's 2026-07-12 status note was waiting on). **Both root-caused claude-code matcher bugs are fixed upstream**, verified on `claude.exe` 2.1.207 (native CLI, isolated sandbox): path-globbed allow/deny rules now match, `defaultMode: bypassPermissions` now activates; fix landed silently in 2.1.145–2.1.207 with no maintainer comment on any tracked thread. Consequences applied: reopen-vs-consolidate on #36884 is moot; drafted native-CLI-scoped resolution notes for #57132 and #15921 (`tasks/working_artifacts/ISSUE-0002/resolution-notes-2026-07-12.md`) — #15921 note explicitly does NOT claim the VS Code extension Bash symptom fixed. Freshness-check and fix-verification acceptance criteria checked off; remaining criteria are post-approval posting + close as "fixed independently upstream". All three DISCUSSIONS.md rows updated. **Awaiting operator approval to post the resolution notes.**
